...
export const AccountRail = memo(function AccountRail({
  ...
  return (
    <div className="w-[52px] h-full shrink-0 flex flex-col items-center py-2 bg-zinc-100 text-zinc-700 border-r border-zinc-200/90 dark:bg-zinc-950 dark:text-zinc-300 dark:border-zinc-850 select-none z-20 overflow-y-auto overflow-x-hidden no-scrollbar transition-colors duration-150">
      ...
      {/* 2. Orta Kısım: Dikey Hesap Listesi (İhtiyaç halinde kendi içinde kaydırılabilir) */}
      <div className="flex-1 min-h-0 w-full overflow-y-auto overflow-x-hidden no-scrollbar flex flex-col items-center gap-2 py-1.5 px-1">
        {accounts.map((acc) => {
          ...
          return (
            <div key={acc.id} className="relative group flex items-center justify-center shrink-0">
              {isSelected && (
                <div className="absolute -left-[10px] top-1/2 -translate-y-1/2 w-1 h-5 rounded-r bg-blue-600 dark:bg-white shadow-[0_0_8px_rgba(37,99,235,0.6)] dark:shadow-[0_0_8px_rgba(255,255,255,0.6)]" />
              )}
              <button
      ...
      <div className="flex flex-col items-center gap-2 w-full shrink-0 mt-auto pt-2 border-t border-zinc-200/70 dark:border-zinc-800/80">
        ...
        {/* Ayarlar Çarkı (Her zaman görünür ve tıklanabilir) */}
        <button
          type="button"
